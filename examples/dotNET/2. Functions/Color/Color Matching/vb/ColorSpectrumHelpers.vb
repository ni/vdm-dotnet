...
Public Class ColorSpectrumHelpers
    Private Shared _plotColorsMap As New Dictionary(Of PictureBox, Collection(Of Color))()
    Private Shared _colorSpectrumMap As New Dictionary(Of PictureBox, Collection(Of Double))()
    ...
    Public Shared Sub PlotColorSpectrum(ByVal pictureBox As PictureBox, ByVal sensitivity As ColorSensitivity, ByVal colorSpectrum As Collection(Of Double))
        _colorSpectrumMap(pictureBox) = colorSpectrum
        _plotColorsMap(pictureBox) = GetPlotColors(sensitivity)
        ' Make sure the PictureBox gets redrawn.
        pictureBox.Invalidate()
    End Sub
    Public Shared Sub DrawColorSpectrum(ByVal g As Graphics, ByVal box As PictureBox)
        ' Clear the picture
        g.Clear(Color.FromKnownColor(KnownColor.Control))

        ' If we have no color spectrum, return.
        If Not _colorSpectrumMap.ContainsKey(box) Then
            Exit Sub
        End If
        Dim colorSpectrum As Collection(Of Double) = _colorSpectrumMap(box)
        Dim plotColors As Collection(Of Color) = _plotColorsMap(box)
        If colorSpectrum.Count = 0 Then
            Exit Sub
        End If

        ' Find the maximum value.
        Dim max As Single = CSng(colorSpectrum(0))
        For Each value As Double In colorSpectrum
            If value > max Then
                max = CSng(value)
            End If
        Next
        ...
        Dim bounds As Rectangle = box.ClientRectangle
        ...
        Dim count As Integer = colorSpectrum.Count
        Dim lineWidth As Single = CSng(width) / count
        Dim xOffset As Single = lineWidth / 2
        For i As Integer = 0 To count - 1
            Dim pen As New Pen(plotColors(i), lineWidth)
            g.DrawLine(pen, New PointF(xOffset + lineWidth * i, height), New PointF(xOffset + lineWidth * i, CSng((height * (1.0R - colorSpectrum(i) / max)))))
            pen.Dispose()
        Next
    End Sub
End Class