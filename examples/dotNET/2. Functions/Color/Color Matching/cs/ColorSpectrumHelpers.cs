...
namespace NationalInstruments.Vision
{
    ...
    {
        private static Dictionary<PictureBox, Collection<Color>> _plotColorsMap = new Dictionary<PictureBox,Collection<Color>>();
        private static Dictionary<PictureBox, Collection<double>> _colorSpectrumMap = new Dictionary<PictureBox, Collection<double>>();
        ...
        public static void DrawColorSpectrum(Graphics g, PictureBox box)
        {
            // Clear the picture
            g.Clear(Color.FromKnownColor(KnownColor.Control));

            // If we have no color spectrum, return.
            if (!_colorSpectrumMap.ContainsKey(box))
            {
                return;
            }
            Collection<double> colorSpectrum = _colorSpectrumMap[box];
            Collection<Color> plotColors = _plotColorsMap[box];
            if (colorSpectrum.Count == 0)
            {
                return;
            }

            // Find the maximum value.
            float max = (float)colorSpectrum[0];
            foreach (double value in colorSpectrum)
            {
                if (value > max)
                {
                    max = (float)value;
            ...
            Rectangle bounds = box.ClientRectangle;
            int width = bounds.Width;
            int height = bounds.Height;

            // Draw a line of the appropriate height and color for each pixel found.
            int count = colorSpectrum.Count;
            float lineWidth = ((float)width)/count;
            float xOffset = lineWidth / 2;
            for (int i = 0; i < count; ++i)
            {
                Pen pen = new Pen(plotColors[i], lineWidth);
                g.DrawLine(pen, new PointF(xOffset + lineWidth * i, height), new PointF(xOffset + lineWidth * i, (float)(height * (1.0 - colorSpectrum[i] / max))));
                pen.Dispose();
            }
        }
    }
